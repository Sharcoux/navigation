...
type NamedProps = { name: string } // string literals extend string by assignability
...
const Navigation = React.forwardRef<HTMLDivElement, Props>(({ active, children, duration = 500 }: Props, ref) => {
  const [width, setWidth] = React.useState(1)
  const fallbackRef = React.useRef<HTMLDivElement>(null)
  const sliderRef = (ref || fallbackRef) as React.RefObject<HTMLDivElement>
  ...
  const childrenElementsArray = React.useMemo(() => {
    ...
    return flatChildren.filter((child): child is NamedChild => React.isValidElement(child) && typeof (child as NamedChild).props?.name === 'string')
  }, [children])
  ...
  React.useLayoutEffect(() => {
    ...
    function animateTo (index: number) {
      ...
      const slideElement = (sliderRef.current?.firstElementChild as HTMLElement) || null
      if (slideElement) {
  ...
  const childrenToDisplay = React.useMemo(() => childrenElementsArray
    ...
    .map(child => (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', justifyContent: 'flex-start', position: 'relative', flex: 1, flexBasis: 0 }} key={child.props.name}>{child}</div>))
  , [childrenElementsArray, current, targetIndex])

  const sliderStyle: React.CSSProperties = React.useMemo(() => ({
    flexGrow: 1,
    flexShrink: 0,
    flexBasis: 'auto',
    alignItems: 'stretch',
  ...
  const slideStyle: React.CSSProperties = React.useMemo(() => ({
    position: 'relative',
    display: 'flex',
    flexDirection: 'row',
    flexWrap: 'nowrap',
  ...
  const onResize = React.useCallback(() => {
    const newWidth = sliderRef.current?.offsetWidth || 0
